 ...
 */
export async function getUserWorkLocations(userId: number): Promise<WorkLocation[]> {
  return db.select({
    id: workLocations.id,
 ...
 */
export async function createWorkLocation(location: Omit<WorkLocation, 'id' | 'createdAt' | 'updatedAt'>): Promise<WorkLocation> {
  const [newLocation] = await db.insert(workLocations)
    .values({
      ...location,
      createdAt: new Date(),
      updatedAt: new Date()
...
export async function assignUserToWorkLocation(userId: number, locationId: number): Promise<boolean> {
  try {
    ...
    await db.insert(userWorkLocations)
      .values({
        userId,
        locationId,
        assignedAt: new Date()
...
export async function clockIn(
  ...
  try {
    ...
    const [locationEntry] = await db.insert(userLocationHistory)
      .values({
        userId,
        eventType: 'clock-in',
        latitude,
...
export async function clockOut(
  ...
  try {
    ...
    const [workLocation] = await db.select()
      .from(workLocations)
    ...
    const totalHoursWorked = (now.getTime() - clockInTime.getTime()) / (1000 * 60 * 60);
